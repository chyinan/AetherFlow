...
assertIncludes(workflowProd, 'default-provider: ${WORKFLOW_OCR_PROVIDER:tesseract}', 'workflow production profile must select a real OCR provider')
assertIncludes(envExample, 'WORKFLOW_OCR_MOCK=false', 'environment template must disable OCR mock')
assertIncludes(envExample, 'WORKFLOW_OCR_PROVIDER=tesseract', 'environment template must select a real OCR provider')
...
assertIncludes(initScript, "Set-EnvValue $content 'WORKFLOW_OCR_PROVIDER' 'tesseract'", 'environment initializer must select a real OCR provider')
