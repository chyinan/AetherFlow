...
public class AuthSessionService {
    ...
    public void storeAccessToken(Long userId, String accessToken, Duration accessTtl) {
        redisTemplate.opsForValue().set(AuthRedisKeys.tokenKey(userId), accessToken, accessTtl);
    }

    public boolean rotateRefreshToken(Long userId,
                                      String expectedRefreshToken,
                                      String replacementRefreshToken,
                                      Duration refreshTtl) {
        if (userId == null || !StringUtils.hasText(expectedRefreshToken)
                || !StringUtils.hasText(replacementRefreshToken)
                || refreshTtl == null || refreshTtl.isZero() || refreshTtl.isNegative()) {
            return false;
        }
        Long result = redisTemplate.execute(
                ROTATE_REFRESH_SCRIPT,
                java.util.List.of(AuthRedisKeys.refreshKey(userId)),
