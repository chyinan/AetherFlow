...
| FINAL-INTEGRATION-STABILIZATION-20260530-P0-DEMO-LOGIN-FALLBACK | Final Integration P0 Demo Login Fallback Stabilization | REVIEW | 陈胤安 | chyinan | feature/FINAL-INTEGRATION-STABILIZATION-20260530-p0-demo-login-fallback | frontend/src/services/api/authApi.ts；docs/agent/tasks/FINAL-INTEGRATION-STABILIZATION-20260530-P0-DEMO-LOGIN-FALLBACK.md；docs/agent/logs/2026-05-30.md；AGENT.md | 否 | npm run build（frontend）通过；git diff --check 通过；冲突标记扫描通过；需浏览器补测 Gateway 未启动时默认账号登录 | 2026-05-30 00:48 |
...
| 2026-05-30 00:48 | FINAL-INTEGRATION-STABILIZATION-20260530-P0-DEMO-LOGIN-FALLBACK | chyinan | 修复默认演示账号 Auth fallback：Gateway/Auth 不可用时 `aether.operator / mock-password` 可进入 Demo，401/429 不降级 | frontend/src/services/api/authApi.ts；docs/agent/tasks/FINAL-INTEGRATION-STABILIZATION-20260530-P0-DEMO-LOGIN-FALLBACK.md；docs/agent/logs/2026-05-30.md；AGENT.md | 分类用例通过；`npm run build` 通过；`git diff --check` 通过；冲突标记扫描通过 | 1efaa82 claim；dff9910 business | 未合入 | 未运行 | 需浏览器补测 Gateway 未启动时默认账号登录；真实 JWT 登录仍需后端服务和种子用户 | RELEASED |
