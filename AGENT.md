...
| 2026-05-30 01:10 | FINAL-INTEGRATION-STABILIZATION-20260530-P1-RUNTIME-LOG-CAP | chyinan | TDD 完成 Runtime 历史日志响应裁剪：`/workflow-instances/{id}/logs` 仅返回最近 200 条，降低长 Whisper 任务日志加载卡顿风险；不改 Runtime Core/SSE | backend/workflow-service/src/main/java/com/aetherflow/workflow/service/impl/WorkflowInstanceQueryServiceImpl.java；backend/workflow-service/src/test/java/com/aetherflow/workflow/service/impl/WorkflowInstanceQueryServiceImplTest.java；docs/agent/tasks/FINAL-INTEGRATION-STABILIZATION-20260530-P1-RUNTIME-LOG-CAP.md；docs/agent/logs/2026-05-30.md；AGENT.md | Red 失败符合预期；目标测试 5 tests 通过；query/controller 回归 8 tests 通过；`git diff --check` 通过；冲突扫描通过 | e823c99 claim；ed7e16c business | 未合入 | 未运行 | 需统一运行电脑补测真实长视频 Run 日志加载和 Runtime Monitor 不明显卡顿 | RELEASED |
